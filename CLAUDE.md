...
- **CLI Entry Point** (`src/cli.ts`): Main command handler using Commander.js
- **Agent Commands** (`src/commands/agent.ts`): Implements `list`, `run`, and `new` subcommands
- **Prompt Discovery** (`src/services/promptDiscovery.ts`): File system scanning and agent resolution
- **VS Code Integration** (`src/services/vscodeIntegration.ts`): Executes `code chat -a` commands
